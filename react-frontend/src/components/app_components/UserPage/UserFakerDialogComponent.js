...
import userFakerFactory from "./userFakerFactory";
...
const UserFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = userFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("user").create(elem));
...
export default connect(mapState, mapDispatch)(UserFakerDialogComponent);
